    ...
    <div className={styles.container}>
      <form onSubmit={handleSubmit} className={styles.card}>
        <div className={styles.tabHeader}>
          <button type="button" onClick={() => setActiveTab('personnel')} className={activeTab === 'personnel' ? styles.active : ''}>Informations personnelles</button>
          <button type="button" onClick={() => setActiveTab('facturation')} className={activeTab === 'facturation' ? styles.active : ''}>Informations de facturation</button>
          <button type="button" onClick={() => setActiveTab('entreprise')} className={activeTab === 'entreprise' ? styles.active : ''}>Informations de votre entreprise</button>
        </div>
        ...
        {activeTab === 'personnel' && (
          <div className={styles.section}>
            ...
            <div className={styles.inlineGroup}>
              <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="Email" readOnly={!editingEmail} className={[!editingEmail && styles.readOnly, editingEmail && (emailValid ? styles.emailValid : styles.emailInvalid)].filter(Boolean).join(' ')} />
              <button type="button" className={styles.editBtn} onClick={() => setEditingEmail((v) => !v)}>{editingEmail ? 'Annuler' : 'Modifier'}</button>
            </div>
            <div className={styles.inlineGroup}>
              <input name="password" type="password" value={form.password} onChange={handleChange} placeholder="Mot de passe (laisser vide si inchangé)" readOnly={!editingPassword} className={[!editingPassword && styles.readOnly, editingPassword && pwdStrength === 'faible' && styles.pwdFaibleBg, editingPassword && pwdStrength === 'moyen' && styles.pwdMoyenBg, editingPassword && pwdStrength === 'fort' && styles.pwdFortBg].filter(Boolean).join(' ')} />
              <button type="button" className={styles.editBtn} onClick={() => setEditingPassword((v) => !v)}>{editingPassword ? 'Annuler' : 'Modifier'}</button>
            </div>
            {editingPassword && (
              <>
                <div className={styles.strengthMeter}>
                  <div className={[styles.strengthBar, pwdStrength === 'faible' && styles.strengthFaible, pwdStrength === 'moyen' && styles.strengthMoyen, pwdStrength === 'fort' && styles.strengthFort].filter(Boolean).join(' ')} />
                </div>
                <small className={styles.strengthLabel}>Force : {pwdStrength}</small>
              </>
            )}
          </div>
        )}
